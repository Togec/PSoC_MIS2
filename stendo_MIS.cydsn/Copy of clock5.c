...
//int rtc_minutes = 59;
#define rtc_minutes (rtc_typef[0] & 0x3F)
...
void rtc_tick(void)
{
    ...
    {
        rtc_seconds = 0;
        rtc_set_minutes(rtc_minutes+1);
        if (rtc_minutes>59)
        {
            rtc_set_minutes(0);
            rtc_set_hour(rtc_hour+1);
            if (rtc_hour>23)
            {
                rtc_set_hour(0);
                rtc_set_day(rtc_day+1);
                if ((rtc_day == 0) || (rtc_day > rtc_days_in_month()))
                {
                    rtc_set_day(1);
                    rtc_set_month (rtc_month+1);
                    if (rtc_month>12)
                    {
                        rtc_set_month (1);
...
char * test_rtc_seconds() {
    rtc_set_seconds(0);
    it("initialize RTC seconds to 0", rtc_seconds == 0);
    rtc_set_seconds(1);
    it("initialize RTC seconds to 1", rtc_seconds == 1);
    rtc_tick();
    it("increment RTC seconds by 1 (1->2)", rtc_seconds == 2);
    rtc_set_seconds(59);
    it("initialize RTC seconds to 0", rtc_seconds == 59);
    rtc_tick();
    it("handle RTC seconds overflow (59->0)", rtc_seconds == 0);
    return 0;
}

char * test_rtc_minutes() {
    rtc_set_minutes(0);
    it("initialize RTC minutes to 0", rtc_minutes == 0);
    rtc_set_minutes(1);
    it("initialize RTC minutes to 1", rtc_minutes == 1);
    rtc_set_seconds(59);
    rtc_tick();
    it("increment RTC minutes by 1 (1->2)", rtc_minutes == 2);
    rtc_set_minutes(59);
    rtc_set_seconds(59);
    rtc_tick();
    it("handle RTC minutes overflow (59->0)", rtc_minutes == 0);
    return 0;
}
    ...
    for (y = 2000; y < 2100; y++)
    {
        ...
        if( ((rtc_year % 4 == 0) && (rtc_year % 100 != 0)) || (rtc_year % 400 == 0) )
        {
            ...
            for (m = lmin; m < lmax; m++)
            {
                ...
                rtc_set_hour(23);
                rtc_tick();
                sprintf(text, "handle end of month overflow, year %d, month %d, day %d, hour %d, minutes %d, seconds %d", rtc_year, rtc_month, rtc_day, rtc_hour, rtc_minutes, rtc_seconds);
                it(text, rtc_seconds == 0);
                it(text, rtc_minutes == 0);
                it(text, rtc_hour == 0);
                it(text, rtc_day == 1);
                if (m < 12)
                {
...
char * test_rtc_typef() {
    ...
    rtc_typef[2]=0x82;
    rtc_typef[3]=0x09;
    it("initialize RTC minutes to 10", rtc_minutes == 10);
...
{
    {"Real Time Clock Seconds", &test_rtc_seconds},
    {"Real Time Clock Minutes", &test_rtc_minutes},
