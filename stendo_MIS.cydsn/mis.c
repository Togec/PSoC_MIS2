...
uint32 PulseCounter_1_Timestamp_hw;
uint32 pulseCounter1 = 0;
CY_ISR(isr_PulseCounter_1_handler)
{
   pulseCounter1++;
   //PulseCounter_1_Timestamp_hw = Timestamp_ReadCounter();
}
...
uint32 MIS1_LastPulseTime(void)
{
    uint32 result = 0;
    
        result  = (capture_1_timestampTimer[0]<<8) | (0xFF & capture_1_Timestamp_hw[0]); /* timestampTimerStart */
        result -= (PulseCounter_1_Timestamp_hw<<8) | (0xFF & PulseCounter_1_Timestamp_hw); /* timestampTimerStop  */
...
{
    return (pulseCounter1<<7) | (0x7F & PulseCounter_1_ReadCounter());
...
{
    return (pulseCounter2<<7) | (0x7F & PulseCounter_2_ReadCounter());
}
...
{
    return (pulseCounter3<<7) | (0x7F & PulseCounter_3_ReadCounter());
}
...
{
    return (pulseCounter4<<7) | (0x7F & PulseCounter_4_ReadCounter());
}
