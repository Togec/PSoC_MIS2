...
/*******************************************************************************
* Function Name: timer_clock_6_Start
********************************************************************************
*
* Summary:
...
void timer_clock_6_Start(void) 
{
    ...
    timer_clock_6_CLKEN |= timer_clock_6_CLKEN_MASK;
	timer_clock_6_CLKSTBY |= timer_clock_6_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_6_Stop
********************************************************************************
*
* Summary:
...
void timer_clock_6_Stop(void) 
{
    ...
    timer_clock_6_CLKEN &= (uint8)(~timer_clock_6_CLKEN_MASK);
	timer_clock_6_CLKSTBY &= (uint8)(~timer_clock_6_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: timer_clock_6_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((timer_clock_6_CLKEN & timer_clock_6_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = timer_clock_6_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = timer_clock_6_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* timer_clock_6__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(timer_clock_6_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(timer_clock_6_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: timer_clock_6_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: timer_clock_6_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = timer_clock_6_GetSourceRegister();
    uint16 oldDivider = timer_clock_6_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = timer_clock_6_CLKEN & timer_clock_6_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(timer_clock_6_DIV_PTR, clkDivider);
                timer_clock_6_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                timer_clock_6_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(timer_clock_6_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((timer_clock_6_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: timer_clock_6_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(timer_clock_6_DIV_PTR);
}


/*******************************************************************************
* Function Name: timer_clock_6_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock_6_MOD_SRC |= modeBitMask & (uint8)timer_clock_6_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_6_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock_6_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(timer_clock_6_MODE_MASK));
}


/*******************************************************************************
* Function Name: timer_clock_6_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return timer_clock_6_MOD_SRC & (uint8)(timer_clock_6_MODE_MASK);
}


/*******************************************************************************
* Function Name: timer_clock_6_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = timer_clock_6_GetDividerRegister();
    uint8 oldSrc = timer_clock_6_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        timer_clock_6_MOD_SRC |= CYCLK_SSS;
        timer_clock_6_MOD_SRC =
            (timer_clock_6_MOD_SRC & (uint8)(~timer_clock_6_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: timer_clock_6_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return timer_clock_6_MOD_SRC & timer_clock_6_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: timer_clock_6_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock_6_PHASE = clkPhase & timer_clock_6_PHASE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_6_GetPhase
********************************************************************************
*
* Summary:
...
{
    return timer_clock_6_PHASE & timer_clock_6_PHASE_MASK;
}
