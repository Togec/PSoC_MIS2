...
/*******************************************************************************
* Function Name: timer_clock_7_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_Start(void) 
{
    /* Set the bit to enable the clock. */
    timer_clock_7_CLKEN |= timer_clock_7_CLKEN_MASK;
	timer_clock_7_CLKSTBY |= timer_clock_7_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_7_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    timer_clock_7_CLKEN &= (uint8)(~timer_clock_7_CLKEN_MASK);
	timer_clock_7_CLKSTBY &= (uint8)(~timer_clock_7_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: timer_clock_7_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_StopBlock(void) 
{
    if ((timer_clock_7_CLKEN & timer_clock_7_CLKEN_MASK) != 0u)
    {
...
#if defined(timer_clock_7__CFG3)
        CLK_DIST_AMASK = timer_clock_7_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = timer_clock_7_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* timer_clock_7__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(timer_clock_7_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(timer_clock_7_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: timer_clock_7_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        timer_clock_7_CLKSTBY &= (uint8)(~timer_clock_7_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: timer_clock_7_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = timer_clock_7_GetSourceRegister();
    uint16 oldDivider = timer_clock_7_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = timer_clock_7_CLKEN & timer_clock_7_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(timer_clock_7_DIV_PTR, clkDivider);
                timer_clock_7_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                timer_clock_7_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(timer_clock_7_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((timer_clock_7_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: timer_clock_7_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 timer_clock_7_GetDividerRegister(void) 
{
    return CY_GET_REG16(timer_clock_7_DIV_PTR);
}


/*******************************************************************************
* Function Name: timer_clock_7_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_SetModeRegister(uint8 modeBitMask) 
{
    timer_clock_7_MOD_SRC |= modeBitMask & (uint8)timer_clock_7_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_7_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_ClearModeRegister(uint8 modeBitMask) 
{
    timer_clock_7_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(timer_clock_7_MODE_MASK));
}


/*******************************************************************************
* Function Name: timer_clock_7_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 timer_clock_7_GetModeRegister(void) 
{
    return timer_clock_7_MOD_SRC & (uint8)(timer_clock_7_MODE_MASK);
}


/*******************************************************************************
* Function Name: timer_clock_7_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = timer_clock_7_GetDividerRegister();
    uint8 oldSrc = timer_clock_7_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        timer_clock_7_MOD_SRC |= CYCLK_SSS;
        timer_clock_7_MOD_SRC =
            (timer_clock_7_MOD_SRC & (uint8)(~timer_clock_7_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: timer_clock_7_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 timer_clock_7_GetSourceRegister(void) 
{
    return timer_clock_7_MOD_SRC & timer_clock_7_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: timer_clock_7_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void timer_clock_7_SetPhaseRegister(uint8 clkPhase) 
{
    timer_clock_7_PHASE = clkPhase & timer_clock_7_PHASE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_7_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 timer_clock_7_GetPhaseRegister(void) 
{
    return timer_clock_7_PHASE & timer_clock_7_PHASE_MASK;
}
